...
// ============================================================
// ⚡ [파이어베이스 버전] 데이터 로더 (기존 GAS 대체 - 심장 교체 완료!)
// ============================================================
...
async function showSalesDashboard() {
  ...
  if(sk) sk.classList.remove('hidden'); // 스켈레톤 보이기
  if(ct) ct.classList.add('hidden');    // 차트 숨기기

  try {
    // ⚡ 파이어베이스에서 데이터 가져와서 가공하기
    const processedData = await fetchAndProcessFirebaseData();
    
    // 기존 차트 초기화 함수에 그대로 전달 (구조가 같으므로 완벽 호환)
    initDashboard({ status: 'success', data: processedData });
...
// 2. [핵심] 파이어베이스 데이터를 -> 차트용 데이터 포맷으로 변환
async function fetchAndProcessFirebaseData() {
    const { collection, getDocs, query, orderBy } = await import("https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js");
    
    // (1) DB에서 날짜순으로 싹 긁어오기
    const q = query(collection(window.db, "daily_sales"), orderBy("__name__", "asc"));
    const snapshot = await getDocs(q);

    // (2) 데이터 그릇 만들기 (GAS가 주던 것과 똑같은 모양)
    const thisYear = new Date().getFullYear(); 
    const lastYear = thisYear - 1;             

    // 초기화 함수
    const initUserStruct = () => ({
        thisYear: Array(12).fill(0),
        thisYearDetails: Array.from({length: 12}, () => ({s:0, r:0})),
        lastYear: Array(12).fill(0),
        lastYearDetails: Array.from({length: 12}, () => ({s:0, r:0})),
        daily: {} // "2025-01-03": { t:10, s:5, r:5 } 형태
    });
    ...
    snapshot.forEach(doc => {
        const dateStr = doc.id; // "2025-10-01"
        const data = doc.data();
        
        // 날짜 파싱
        const [yStr, mStr, dStr] = dateStr.split('-');
        const year = parseInt(yStr, 10);
        const monthIdx = parseInt(mStr, 10) - 1; // 0~11
        
        // 담당자별 데이터 처리
        ['김원대', '정병준'].forEach(kName => {
            const eName = (kName === '김원대') ? 'kim' : 'jung';
            const raw = data[eName] || { st: 0, rice: 0 }; // DB 필드명 (st, rice)

            const qty1 = raw.st || 0;   // 생탁
            const qty2 = raw.rice || 0; // 우리쌀
            const total = qty1 + qty2;

            if (total > 0) {
                // A. 일별 데이터 (Daily)
                salesData[kName].daily[dateStr] = { t: total, s: qty1, r: qty2 };

                // B. 월별 누적 데이터 (Yearly)
                if (year === thisYear) {
                    salesData[kName].thisYear[monthIdx] += total;
                    salesData[kName].thisYearDetails[monthIdx].s += qty1;
                    salesData[kName].thisYearDetails[monthIdx].r += qty2;
                } else if (year === lastYear) {
                    salesData[kName].lastYear[monthIdx] += total;
                    salesData[kName].lastYearDetails[monthIdx].s += qty1;
                    salesData[kName].lastYearDetails[monthIdx].r += qty2;
                }
            }
        });
    });

    return salesData; // 완성된 데이터 반환
}
